...
function buildSetupSkillContent() {
  return `# setup-homekit

Bootstrap or re-bootstrap the HomeKit integration with the homebridge-openclaw-gateway plugin.

## Usage

\`\`\`
/setup-homekit <plugin-url> <bootstrap-token>
\`\`\`

## Steps

1. Call \`GET <plugin-url>/api/setup\` with \`Authorization: Bearer <bootstrap-token>\`.

2. Write each entry in \`skills\` to its \`path\` (relative to workspace root). Overwrite any existing file.

3. Write each entry in \`triggers\` to its \`path\`. Overwrite any existing file.

4. Append \`claude_md_addition\` to \`CLAUDE.md\` only if the plugin header line is not already present. Do not duplicate it.

5. Write env vars from \`env\` to settings.

6. Upsert the cron job — the setup response includes a \`cron.name\` field for identification:
   - List all existing cron jobs.
   - Search for a job whose \`name\` exactly matches \`cron.name\` from the setup response.
   - **If a matching job is found**: update it (\`action: "update"\`, \`id: <job-id>\`), patching only \`sessionTarget\` and \`schedule\` from the response. Leave the existing payload message untouched to preserve any local customisation.
   - **If no matching job is found**: add a new job (\`action: "add"\`) with \`name\`, \`schedule\`, \`sessionTarget\`, and \`lightContext: true\` from the response, and a message of \`Handle the scheduled \\\`/homekit-events\\\` check. After the poll respond with exactly \\\`NO_REPLY\\\`.\`

7. Report to the user: files written, whether the cron was created or updated, and the final job ID.
`;
}

module.exports = { buildSkillContent, buildGarageTriggerContent, buildClaudeMdAddition, buildSetupSkillContent };
