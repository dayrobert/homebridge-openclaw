...
const { buildSkillContent, buildGarageTriggerContent, buildClaudeMdAddition, buildSetupSkillContent } = require('./setup-bundle');
...
function setupRoutes(app, apiAuth, uiClient, roomStore, eventQueue, apiConfig = {}) {
  ...
  app.get('/api/setup', apiAuth.requireBootstrapOrSession, (_req, res) => {
    res.json({
      ...
      skills: [
        { name: 'homekit-events', path: '.openclaw_gateway/commands/homekit-events.md', content: buildSkillContent(externalUrl, apiAuth.sessionTtlSeconds) },
        { name: 'setup-homekit', path: '.openclaw_gateway/commands/setup-homekit.md', content: buildSetupSkillContent() },
      ],
      ...
      cron: { name: 'HomeKit events', schedule: '* * * * *', command: '/homekit-events', description: 'Poll HomeKit for state changes every minute', sessionTarget: 'isolated' },
